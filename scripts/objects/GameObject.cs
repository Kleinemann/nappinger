...
{
    [Export] public ObjectBaseResource _Obj;

    public Texture2D Texture => _Obj.Texture;
    public int ID => _Obj.ID;
    public string ObjectName => _Obj.Name;

    public override void _Ready()
    {
        Sprite2D sprite = GetNode<Sprite2D>("Sprite2D");
        Sprite2D spriteShadow = GetNode<Sprite2D>("Sprite2DShadow");
        sprite.Texture = _Obj.Texture;
        spriteShadow.Texture = _Obj.Texture;

        int defaultSize = Chunk.TileSize;

        Vector2 size = _Obj.Texture.GetSize();
        CollisionShape2D shape = GetNode<CollisionShape2D>("CollisionShape2D");
        RectangleShape2D rect = shape.Shape as RectangleShape2D;
        rect.Size = size;

        spriteShadow.Position = new Vector2I(3, defaultSize - ((int)size.Y / 2) - 3);
    }
